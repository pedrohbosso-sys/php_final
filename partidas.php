...
/* BUSCA DADOS */
$times = $pdo->query("SELECT id, nome FROM times")->fetchAll(PDO::FETCH_ASSOC);
$campeonatos = $pdo->query("SELECT id, nome FROM campeonatos")->fetchAll(PDO::FETCH_ASSOC);
...
/* LISTA PARTIDAS (TODOS PODEM VER) */
$partidas = $pdo->query("
    SELECT p.id, 
           t1.nome AS time1, 
           t2.nome AS time2, 
    ...
    <div class="container-wide">
        ...
        <?php if ($isAdmin): ?>
            ...
            <form method="POST">
                ...
                <input type="number" name="placar1" placeholder="Placar Time 1" min="0" required>
                ...
                <input type="number" name="placar2" placeholder="Placar Time 2" min="0" required>
        ...
        <table>
            ...
            <?php foreach ($partidas as $p): ?>
                <tr>
                    ...
                    <td><?= htmlspecialchars($p['time1']) ?></td>
                    <td class="placar"><?= $p['placar1'] ?> x <?= $p['placar2'] ?></td>
                    <td><?= htmlspecialchars($p['time2']) ?></td>
